...
begin
  ...
  begin
    ...
    StrPCopy(szTip, AppName+' v'+AppVersion+' - '+AppDesc);                //Указываем текст всплывающей
...
begin
   ...
   if Key = VK_RIGHT then if ((Shape1.Left<370) or (Shape1.Top>195) and (Shape1.Left<420)) then Shape1.Left := Shape1.Left + 52;

   if Key = VK_LEFT then if Shape1.Left>10 then Shape1.Left := Shape1.Left - 52;

   if Key = VK_DOWN then if Shape1.Top <195 then Shape1.Top := Shape1.Top + 65;

   if Key = VK_UP then if Shape1.Top>10 then Shape1.Top := Shape1.Top - 65;
...
begin
  ...
  if (length(MyRegReadString(HKEY_CURRENT_USER, '\Software\' + AppName, 'AutoUpdates')) > 0)
  then
  begin
...
begin
  ...
  if (length(MyRegReadString(HKEY_CURRENT_USER, '\Software\' + AppName, 'AutoUpdates')) > 0)
    then cm_checkupdates.Checked := true
    else cm_checkupdates.Checked := false;

//    showmessage (inttostr(DaysBetween(Now(), Reg_ReadDateTime(HKEY_CURRENT_USER, '\Software\' + AppName,'LastUpdate'))));

  // Делать ли проверку обновлений (должно идти до напоминалки!)
  if ((cm_checkupdates.Checked) and
      (DaysBetween(Now(), Reg_ReadDateTime(HKEY_CURRENT_USER, '\Software\' + AppName,'LastUpdate')) > DaysBetweenUpdate))
      then IsNeedCheckUpdates := True
      else IsNeedCheckUpdates := False;

  // Напоминалка про обновлении (должно идти после проверки необходимости проверки обновлений!)
  if (length(MyRegReadString(HKEY_CURRENT_USER, '\Software\' + AppName, 'NewVersionExists')) > 0) then
    if (MyRegReadString(HKEY_CURRENT_USER, '\Software\' + AppName, 'NewVersionExists') = AppVersion)
      then MyRegDelete(HKEY_CURRENT_USER, '\Software\' + AppName,'NewVersionExists')
      else begin
            updlabel.Visible:=true;
            updlabel.Caption := 'Доступна новая версия программы ('+MyRegReadString(HKEY_CURRENT_USER, '\Software\' + AppName, 'NewVersionExists')+')';
...
begin
  ...
  // Цвет формы
  if (length(MyRegReadString(HKEY_CURRENT_USER, '\Software\' + AppName, 'WindowColor')) > 0)
    then Form1.Color := StrToInt(MyRegReadString(HKEY_CURRENT_USER, '\Software\' + AppName, 'WindowColor'))
    else Form1.Color := clSkyBlue;

  // Скрывать ли форму после вставки каждого символа
  IsHideAfterInsert := false;
  if (length(MyRegReadString(HKEY_CURRENT_USER, '\Software\' + AppName, 'FrmHideToggle')) > 0)
    then if (MyRegReadString(HKEY_CURRENT_USER, '\Software\' + AppName, 'FrmHideToggle') = 'no') then IsHideAfterInsert := true;


  // Горячие клавиши
  UnRegisterHotkey( Handle, 1 );
  if (MyRegReadInteger(HKEY_CURRENT_USER, '\Software\' + AppName, 'HotKey0') = 0)
    then
      RegisterHotkey(Handle, 1, MOD_ALT, ord('Z'))
    else
      if not RegHotKey(MyRegReadInteger(HKEY_CURRENT_USER, '\Software\' + AppName, 'HotKey0'), 1) then
         ShowMessage('Не могу назначить горячую клавишу!');

end;

procedure TForm1.IconCallBackMessage( var Mess : TMessage );
var cursorPosition: TPoint;
begin
  case Mess.lParam of
...
begin
    ...
    if IsHideAfterInsert then begin
     Form1.Show;
     SetForegroundWindow(Application.Handle);
    end;
//    Form1.SendToBack;
...
begin
  ...
  RefreshUpdateFlag();
  if (IsNeedCheckUpdates) then TryCheckUpdatesIfNeeded();
end;
...
//    Synchronize(ShowResult1);
      ...
      begin
        ...
        if (AppVersion<>Ver) then
          begin
           // обновиться надо
           Form1.MyRegWriteString(HKEY_CURRENT_USER, '\Software\' + AppName, 'NewVersionExists', Ver);
